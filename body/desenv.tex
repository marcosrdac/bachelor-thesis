...
que formam um vetor $\vec{y_n}$, o qual se relaciona à matriz $\boldsymbol{Y}_{N \times Q}$, tal como $\vec{x}_n$ se relaciona a $\boldsymbol{X}_{N \times M}$:
  ...
  \section{Atualização de parâmetros} \label{s:metodos_gradiente}
    ...
    \subsection{Estimativa adaptável de momento (Adam)} \label{s:adam}
      ...
      Como as médias móveis são inicializadas nulas, as estimativas dos momentos é inicialmente enviesada. Para reduzir o viés nos valores estimados, devemos dividi-las por $(1-\beta^i)$ \shortcite{kingma2014adam}, onde $\beta$ representa a taxa de decaimento associada ao momento em questão e $i$ é o número da iteração de otimização.
